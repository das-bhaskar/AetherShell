...
- **Hurdle:** Even with a simplified 16-bit bitmap, the CPU overhead of scanning the entire VRAM bank and the subsequent TCP overhead of sending thousands of redundant points resulted in a 15–20 second delay per frame.
...
- **The Fix:** I implemented `lcdMainOnTop()` to physically swap the engine output.
...
To overcome the "blobby" look of standard 8-bit modes, I utilized `BgType_Bmp16`.
...
To prevent "ghost lines" (diagonal streaks connecting separate strokes), I utilized the `keysUp(KEY_TOUCH)` state.
...
- **Fix:** In `network.cpp`, I implemented a **30-frame VBlank Linger** (`for(int i=0; i<30; i++) swiWaitForVBlank();`) after the `send()` call. This allows the ARM7 co-processor enough time to empty the Wi-Fi TX buffer before `close(sock)` is called.
